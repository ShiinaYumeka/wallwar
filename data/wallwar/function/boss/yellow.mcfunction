...
execute store result score #boss_team temp if entity @a[team=yellow]
execute store result score #other_team temp if entity @a[team=!sp,team=!yellow]

execute as @a[team=yellow] run function wallwar:boss/change
...
#复活惩罚
scoreboard players set @a[team=yellow] recipe 2